...
//Mandelbox
void sphereFold(vec3 * z, frac * dz, int * nummy ) 
{
...
frac distanceFunction( frac x, frac y, frac z, frac time, vec4 * color )
{
	...
	for (n = 0; n < Iterations && continuing; n++) {
		...
		sphereFold(&point,&dr,&nam);    // Sphere Inversion
 		nom++;
		
		point=Scale*point + offset;  // Scale & Translate
		
		trap += dot(point,point)/5;
		po += point;
		
		dr = dr*fabs(Scale)+1.0;
		
		continuing = dot( point,point) < 160000; 
	}
	po = normalize(offset)/4 + (vec3)(1.5,1.5,1.5);
	/**color = (vec4)(hsvToRgb( (vec3)( ((frac)(n))/Iterations,1,1) ), 1 );
	*color = (vec4)(hsvToRgb( (vec3)( trap/Iterations,1,1 ) ), 1 );
	*color = (vec4)( po.x, po.y, po.z, 1 );*/
	vec3 col = normalize( fabs( point ) );
...
// {
	...
	// for (int i = 0; i < Iterations ; i++) {
		...
		// frac theta = asin(point.z/r);
		// frac phi = atan2(point.y,point.x);
		...
		// point = zr*(vec3)(cos(theta)*cos(phi), sin(phi)*cos(theta), sin(theta));
		// point+=(vec3)(x,y,z);
	// }
...
vec3 jankGetNormal( vec3 position, vec3 rayDirection, frac dist, frac limit )
{
	...
	return normalize( (vec3)(
		distanceFunction(position.x+dist,position.y,position.z, 0, &colTemp ) - distanceFunction(position.x-dist,position.y,position.z, 0, &colTemp ),
		distanceFunction(position.x,position.y+dist,position.z, 0, &colTemp ) - distanceFunction(position.x,position.y-dist,position.z, 0, &colTemp ),
		distanceFunction(position.x,position.y,position.z+dist, 0, &colTemp ) - distanceFunction(position.x,position.y,position.z-dist, 0, &colTemp ) ) );
}
	...
	if( idx < (dimensions.x * dimensions.y) )
	{
		...
		{			
			...
			if( intersect )
			{
				...
				vec3 normal = jankGetNormal( currentRay, rayDirection, leastMarch/2, leastMarch );
